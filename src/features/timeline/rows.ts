...
export function buildTimelineTreeRows(params: {
  ...
  const visit = (parentId: string, level: number) => {
    ...
    for (const child of children) {
      ...
      if (params.shouldHideNode?.(child)) continue;
      if (
      ...
      const hasChildren = params.isHiddenExecutionTaskNode(child)
        ? false
        : (params.byParent.get(child.id) ?? []).some(
            (grandchild) => !isFileLikeNode(grandchild) && !params.shouldHideNode?.(grandchild)
          );
      rows.push({
        id: child.id,
        node: child,
  ...
  if (params.activeProjectRootId) {
    ...
    const rootHasChildren = (params.byParent.get(rootNode.id) ?? []).some(
      (child) => !isFileLikeNode(child) && !params.shouldHideNode?.(child)
    );
    rows.push({
      id: rootNode.id,
      node: rootNode,
...
export function buildFilteredScheduleTimelineRows(params: {
  ...
  const visitNode = (nodeId: string, level: number): { branchVisible: boolean; rows: TimelineTreeRow[] } => {
    ...
    if (params.shouldHideNode?.(node)) return { branchVisible: false, rows: [] };
    if (params.isHiddenExecutionTaskNode(node) && !params.isRenderableExecutionProjectionNode(node)) {
      return { branchVisible: false, rows: [] };
    }

    const children = params.isHiddenExecutionTaskNode(node)
      ? []
      : params
          .sortTimelineNodesForDisplay(params.byParent.get(nodeId) ?? [])
          .filter((child) => !params.shouldHideNode?.(child));
    let hasVisibleChild = false;
    const childRows: TimelineTreeRow[] = [];
    for (const child of children) {
  ...
  const visitChildren = (parentId: string, level: number): TimelineTreeRow[] => {
    const rows: TimelineTreeRow[] = [];
